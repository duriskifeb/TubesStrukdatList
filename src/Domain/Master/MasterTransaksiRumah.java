...
public class MasterTransaksiRumah {
    final TransaksiDataSource dataSource;

    public MasterTransaksiRumah(TransaksiDataSource dataSource) {
    ...
    private boolean cekTransaksi(String reportNumber) {
        Transaksi cek = dataSource.getListTransaksi().stream().filter(
                transaksi -> transaksi.getNoTransaksi().equals(reportNumber)).findFirst().orElse(null);
